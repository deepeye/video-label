...
2. **审核改动必须回写到 Zustand store**。时间轴与事件编辑器只是渲染，创建/编辑/删除事件、修改事件属性必须同步更新内存中的事件数据。否则步骤⑤导出拿不到客户的改动 → Demo 灵魂崩塌。
...
- **拖入任意视频**：用 `URL.createObjectURL` 真实播放该文件画面，但**标注数据仍套用预置 Mock**（默认引导用预置样例规避「真画面 + 假数据」不一致）。

## 实现顺序建议（PRD 第十一章）

1. **先建 Mock 数据集 + Demo 编排状态机**（骨架）
2. **再做步骤④审核工作台**（最重 + Demo 高点：时间轴事件标记 + 事件编辑器 + 内存状态更新）
